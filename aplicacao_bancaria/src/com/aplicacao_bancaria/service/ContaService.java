...
public class ContaService {
    ...
    public String transferir(String origem, String destino, double valor) {
        ...
        if (origem.equals(destino)) return "Contas iguais.";

        Conta contaOrigem = repository.buscarPorNumero(origem);
        Conta contaDestino = repository.buscarPorNumero(destino);

        if (contaOrigem == null || contaDestino == null) return "Conta inválida.";
        ...
        if (!contaOrigem.sacar(valor)) return "Saldo insuficiente.";

        contaDestino.setSaldo(contaDestino.getSaldo() + valor);

        contaOrigem.getHistorico().add(
            new Transacao("TRANSFERÊNCIA", valor, "Enviado para " + destino)
        );

        contaDestino.getHistorico().add(
            new Transacao("TRANSFERÊNCIA", valor, "Recebido de " + origem)
        );
